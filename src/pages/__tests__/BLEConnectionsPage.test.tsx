...
const mockUseBLEProfiles = useBLEProfiles as jest.MockedFunction<typeof useBLEProfiles>;
...
describe('BLEConnectionsPage', () => {
  ...
  describe('Disconnected State', () => {
    it('should render header correctly', () => {
      ...
      expect(screen.getByText('Manage Bluetooth upstream connections')).toBeInTheDocument();
    });

    it('should show connect message when not connected', () => {
      renderComponent();
      expect(screen.getByText('Connect your keyboard to manage BLE profiles')).toBeInTheDocument();
    });

    it('should show help text with colored border description', () => {
      renderComponent();
      expect(screen.getByText(/highlighted with a colored border/)).toBeInTheDocument();
    });
  ...
  describe('Connected State', () => {
    ...
    it('should apply active profile styling', () => {
      renderComponent({ isConnected: true }, { profiles: mockProfiles });
      const activeProfileCard = screen.getByText('Work Laptop').closest('.glass-card');
    ...
    it('should not show refresh button emoji', () => {
      ...
      expect(refreshButton.textContent).not.toContain('🔄');
    });
  });
  ...
  describe('Profile Actions', () => {
    it('should show unpair button for paired profiles', () => {
      ...
      expect(unpairButtons).toHaveLength(2); // Only for paired profiles
    });

    it('should show switch button for inactive profiles', () => {
      renderComponent({ isConnected: true }, { profiles: mockProfiles });
      const switchButtons = screen.getAllByRole('button', { name: /switch/i });
      expect(switchButtons).toHaveLength(2); // Only for non-active profiles
    });

    it('should not show switch button for active profile', () => {
      renderComponent({ isConnected: true }, { profiles: mockProfiles });
      const activeProfileCard = screen.getByText('Work Laptop').closest('.glass-card');
      expect(activeProfileCard?.querySelector('button[aria-label*="Switch"]')).not.toBeInTheDocument();
    });
    ...
    it('should call switchProfile when switch button is clicked', async () => {
      ...
      const switchButton = screen.getAllByRole('button', { name: /switch/i })[0];
    ...
    it('should call unpairProfile with confirmation when unpair is clicked', async () => {
      ...
      await waitFor(() => {
        expect(global.confirm).toHaveBeenCalledWith('Are you sure you want to unpair this device?');
  ...
  describe('Profile Name Editing', () => {
    it('should show edit button for paired profiles', () => {
      ...
      expect(editButtons).toHaveLength(2); // Only for paired profiles
    });

    it('should not show edit button for empty profiles', () => {
      renderComponent({ isConnected: true }, { profiles: mockProfiles });
      const profileCard = screen.getByText('Not paired').closest('.glass-card');
      expect(profileCard?.querySelector('button[aria-label="Edit name"]')).not.toBeInTheDocument();
    });
  ...
  describe('Error Handling', () => {
    it('should display error message when error occurs', () => {
      ...
      expect(screen.getByText('⚠️ Failed to load profiles')).toBeInTheDocument();
    });
  ...
  describe('Button Layout', () => {
    it('should maintain consistent button alignment with spacer for active profile', () => {
      ...
      const activeProfileCard = screen.getByText('Work Laptop').closest('.glass-card');
      const spacer = activeProfileCard?.querySelector('div[aria-hidden="true"]');
